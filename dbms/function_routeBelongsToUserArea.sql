...
BEGIN
	...
	--raise notice 'full array: %', route_points;
	--raise notice 'array length 1: %', jsonb_array_length('[{"latitude":1.2,"longitude":1.2},{"latitude":1.3,"longitude":1.4}]');													
    --raise notice 'array lenght 2: %', jsonb_array_length(formatted_points);													
    --raise notice 'array element: %', formatted_points->0;													

	--SELECT ST_MakePolygon(ST_GeomFromText('LINESTRING(75.15 29.53 1,77 29 1,77.6 29.5 1, 75.15 29.53 1)'));

	--select points from Route where identifier = 4
	...
	THEN													
		FOR json_point IN SELECT * FROM json_array_elements(formatted_points)
		  LOOP
			...
			RAISE NOTICE 'output %', current_point;
		  END LOOP;
		RETURN FALSE;
	END IF;
...
--select routeBelongsToUserArea(0,0,0,0, '[{"latitude":1.2,"longitude":1.2},{"latitude":1.3,"longitude":1.4}]', false, false);
